...
func (e *Engine) generateCommitterConfigs() error {
	...
	for _, component := range e.config.Committer.Components {
		...
		// Note: Original Ansible config does NOT copy genesis.block for sidecar
		// The genesis-block-file-path in config-sidecar.yml is left empty
		// So we don't copy it here to match the original behavior
...
func (e *Engine) buildOrdererTemplateData(ordererType string, org *config.OrdererOrg, node *config.Node, configDir, cryptoDir, genesisDir string, partyID int) *OrdererTemplateData {
	// Determine HSM token label
	tokenLabel := e.getHSMTokenLabel(org)

	// Determine HSM library path (use container path)
	hsmLibPath := e.getHSMLibraryPath()
	...
	data := &OrdererTemplateData{
		...
		HSM: HSMConfig{
			Enabled:  e.config.HSM != nil && e.config.HSM.Enabled,
			Library:  hsmLibPath,
			Pin:      e.getHSMPin(),
			Label:    tokenLabel,
			Hash:     "SHA2",
...
// Helper functions
func (e *Engine) getHSMTokenLabel(org *config.OrdererOrg) string {
	if org.HSMTokenLabel != "" {
		return org.HSMTokenLabel
	}
	if e.config.HSM != nil && e.config.HSM.MultiToken {
		// Match Ansible's format: "FabricToken-Ordererorg1" (lowercase 'org')
		// Ansible uses: "FabricToken-Ordererorg{{ orderer_group }}"
		orgNameLower := strings.ToLower(org.Name)
		return fmt.Sprintf("%s-%s", e.config.HSM.TokenLabel, orgNameLower)
	}
	if e.config.HSM != nil {
		return e.config.HSM.TokenLabel
	}
	return "FabricToken"
}

func (e *Engine) getHSMLibraryPath() string {
	if e.config.HSM != nil && e.config.HSM.ContainerLibPath != "" {
		return e.config.HSM.ContainerLibPath
	}
	if e.config.HSM != nil && e.config.HSM.LibraryPath != "" {
		return e.config.HSM.LibraryPath
	}
	return "/usr/lib64/softhsm/libsofthsm2.so" // Default
}

func (e *Engine) getHSMPin() string {
	if e.config.HSM != nil && e.config.HSM.PIN != "" {
		return e.config.HSM.PIN
	}
	return "1234" // Default
}
...
func (e *Engine) copyDir(src, dst string) error {
	return filepath.Walk(src, func(path string, info os.FileInfo, err error) error {
		...
		// Skip config.yaml file in MSP directory (Ansible doesn't copy it)
